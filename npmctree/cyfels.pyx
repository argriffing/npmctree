...
def iid_likelihoods(
        ...
        cnp.float64_t[:] lhood_out, # (nsites,)
        int validation=1,
        ):
    """
    Compute likelihoods for multiple sites.
    ...
    root_distn : ndarray view
        Distribution over root states.
    lhood_out : ndarray view
        For each site, the likelihood.
    validation : int
        Indicates whether to check the input formats.
        This would include checking the csr representation of the tree and
        the dimensions of the inputs.
    ...
    # Allocate workspace for partial likelihoods.
    ...
    with nogil:
        ...
        for c in range(nsites):
            ...
            # Clear the partial likelihood array.
            for i in range(nnodes):
                for j in range(nstates):
                    lhood[i, j] = 0
            ...
            # Define the site likelihood.
            for sa in range(nstates):
                lhood_out[c] += lhood[0, sa]
